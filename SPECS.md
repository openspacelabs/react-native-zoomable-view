...
3. **Four permanent listeners:** the constructor attaches `panListenerId` and `zoomListenerId`; `componentDidUpdate` attaches `panTransformListenerId` and `zoomTransformListenerId`. All four stay registered for the component's lifetime and fire on every gesture frame, writing derived values back into the external animated object.

Consequences for shared-value patterns (synchronized carousels, interpolated overlays, multi-instance zoom sync): a value shared between this component and another consumer will be reset twice on mount (once by each component's constructor), overwritten on every gesture frame by each component's listeners, and snapped to `initialZoom` whenever any consumer toggles `zoomEnabled` off. There is no prop to opt out of these side effects.
