...
| `doubleTapDelay` | `number` | `300` | Max ms between taps for double-tap detection. **`0` silently disables double-tap detection entirely** — the truthy guard in `_resolveAndHandleTap` short-circuits, so every tap is treated as a single tap. Visual tap feedback is also suppressed by the same falsy-guard pattern. (Pre-existing behavior, not introduced by this PR.) |
