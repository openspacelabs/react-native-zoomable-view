...
| `onDoubleTapBefore` | Before double-tap zoom executes | JS thread (called inline from `_handleDoubleTap`, which runs after a `runOnJS` hop from the gesture worklet). |
| `onDoubleTapAfter` | After double-tap zoom is initiated. The `zoomLevel` field is the **target** zoom, not the pre-animation value | JS thread. Fires synchronously *before* the `withTiming` animation runs, so consumers reading `ref.current.gestureStarted` at this point still see the gesture. |
