...
Whether a gesture is currently in progress. Useful for consumers to suppress their own updates during active interaction. **Caveat:** `gestureStarted` is reset to `false` as the **last** operation of `_handlePanResponderEnd` — it remains `true` throughout the entire end-callback sequence (`onPanResponderEnd`, `onZoomEnd`/`onShiftingEnd`, and the immediate `onStaticPinPositionChange` fired from `_updateStaticPin`). Consumers cannot read `gestureStarted` inside these callbacks to distinguish "gesture ending" from "mid-gesture."
