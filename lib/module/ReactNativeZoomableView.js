...
class ReactNativeZoomableView extends Component {
  ...
  zoomTo(newZoomLevel, coords = {
    ...
    const listenerId = this.zoomAnim.addListener(({
      ...
      this.panAnim.setValue({
        x: calcNewScaledOffsetForZoomCentering(this.offsetX, this.state.originalWidth, prevScale, newScale, coords.x),
        y: calcNewScaledOffsetForZoomCentering(this.offsetY, this.state.originalHeight, prevScale, newScale, coords.y)
      });
      prevScale = newScale;
    });
