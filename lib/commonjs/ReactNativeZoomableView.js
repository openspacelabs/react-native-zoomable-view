...
class ReactNativeZoomableView extends _react.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const staticPinPositionChanged = prevProps.staticPinPosition?.x !== this.props.staticPinPosition?.x || prevProps.staticPinPosition?.y !== this.props.staticPinPosition?.y; // We use a custom `onLayout` event, so the clients can stay in-sync
  ...
  zoomTo(newZoomLevel, zoomCenter) {
    ...
    this.props.onZoomBefore?.(null, null, this._getZoomableViewEventObject()); // == Perform Pan Animation to preserve the zoom center while zooming ==
