...
class ReactNativeZoomableView extends _react.Component {
  ...
  zoomTo(newZoomLevel, coords = {
    ...
    const listenerId = this.zoomAnim.addListener(({
      ...
      this.panAnim.setValue({
        x: (0, _helper.calcNewScaledOffsetForZoomCentering)(this.offsetX, this.state.originalWidth, prevScale, newScale, coords.x),
        y: (0, _helper.calcNewScaledOffsetForZoomCentering)(this.offsetY, this.state.originalHeight, prevScale, newScale, coords.y)
      });
      prevScale = newScale;
    });
