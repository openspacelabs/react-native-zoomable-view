...
class ReactNativeZoomableView extends _react.Component {
  ...
  constructor(props) {
    ...
    this.panListenerId = this.panAnim.addListener(({
      x,
      y
    }) => {
      this.offsetX = x;
    ...
    this.zoomListenerId = this.zoomAnim.addListener(({
      value
    }) => {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (!this.onTransformInvocationInitialized && this._invokeOnTransform().successful) {
      this.panTransformListenerId = this.panAnim.addListener(() => this._invokeOnTransform());
      this.zoomTransformListenerId = this.zoomAnim.addListener(() => this._invokeOnTransform());
  ...
  componentWillUnmount() {
    ...
    if (this.panTransformListenerId) this.panAnim.removeListener(this.panTransformListenerId);
    if (this.zoomTransformListenerId) this.zoomAnim.removeListener(this.zoomTransformListenerId);
    // Clean up zoomTo() listener if mid-animation at unmount
    if (this.zoomToListenerId) this.zoomAnim.removeListener(this.zoomToListenerId);
  }
