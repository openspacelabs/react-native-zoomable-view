...
const ReactNativeZoomableView = (props, ref) => {
  ...
  const lastGestureCenterPosition = (0, _react.useRef)();
  ...
  onLayout.current = props.onLayout; // Handle original measurements changed
  ...
  }, [originalHeight, originalWidth, originalPageX, originalPageY, originalX, originalY]); // Handle staticPinPosition changed

  (0, _react.useLayoutEffect)(() => {
    if (onTransformInvocationInitialized.current) _invokeOnTransform();
  }, [props.staticPinPosition?.x, props.staticPinPosition?.y]);
  (0, _react.useEffect)(() => {
    measureZoomSubject(); // We've already run `grabZoomSubjectOriginalMeasurements` at various events
  ...
  const debouncedOnStaticPinPositionChange = (0, _useDebounceCallback.useDebounceCallback)(position => props.onStaticPinPositionChange?.(position), 100);
  /**
  ...
  const _handlePinching = (0, _useLatestCallback.useLatestCallback)((e, gestureState) => {
    ...
    if (!distance) return;
    if (!lastGestureTouchDistance.current) return; // define the new zoom level and take zoom level sensitivity into consideration
    ...
    let newZoomLevel = zoomLevel.current * (1 + deltaGrowthAdjustedBySensitivity); // make sure max and min zoom levels are respected
  ...
  const _resolveAndHandleTap = (0, _useLatestCallback.useLatestCallback)(e => {
    ...
    if (doubleTapFirstTapReleaseTimestamp.current && props.doubleTapDelay && now - doubleTapFirstTapReleaseTimestamp.current < props.doubleTapDelay) {
      doubleTapFirstTap.current && _addTouch({ ...doubleTapFirstTap.current,
        id: now.toString(),
        isSecondTap: true
      });
      ...
      _addTouch(doubleTapFirstTap.current); // persist event so e.nativeEvent is preserved after a timeout delay


      e.persist();
      singleTapTimeoutId.current = setTimeout(() => {
        delete doubleTapFirstTapReleaseTimestamp.current;
        delete singleTapTimeoutId.current; // Pan to the tapped location
  ...
  const _handleDoubleTap = (0, _useLatestCallback.useLatestCallback)(e => {
    ...
    if (nextZoomStep == null) return; // define new zoom position coordinates
  ...
  const publicZoomTo = (0, _useLatestCallback.useLatestCallback)((newZoomLevel, zoomCenter) => {
    ...
    props.onZoomBefore?.(null, null, _getZoomableViewEventObject()); // == Perform Pan Animation to preserve the zoom center while zooming ==

    let listenerId = '';

    if (zoomCenter) {
      // Calculates panAnim values based on changes in zoomAnim.
      let prevScale = zoomLevel.current; // Since zoomAnim is calculated in native driver,
  ...
  return /*#__PURE__*/(0, _jsxRuntime.jsxs)(_reactNative.View, {
    ...
    children: [/*#__PURE__*/(0, _jsxRuntime.jsx)(_reactNative.Animated.View, {
      style: [styles.zoomSubject, props.style, {
        transform: [// In RN79, we need to split the scale into X and Y to avoid
